...
Silence Protocol:
  All listeners check bot.state["is_setup_active"]. If True, they abort
  immediately to prevent audit-log spam during mass initialization phases.
...
class AuditLogger(commands.Cog):
    ...
    def _is_setup_active(self) -> bool:
        """Return True if the setup/nuke phase is currently running."""
        return self.bot.state.get("is_setup_active", False)
    ...
    async def log(self, guild: discord.Guild, embed: discord.Embed):
        ...
        Silenced during setup phase.
        """
        if self._is_setup_active():
            return
